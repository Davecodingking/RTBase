...
public:
	...
	Ray generateRay(float x, float y)
	{
		...
		Vec3 ndcPoint(ndcX, ndcY, 1.0f);

		
		Vec3 viewPoint = inverseProjectionMatrix.mulPointAndPerspectiveDivide(ndcPoint);

		
		Vec3 worldPoint = camera.mulPointAndPerspectiveDivide(viewPoint);
...
public:
	...
	Light* sampleLight(Sampler* sampler, float& pmf)
	{
		...
		// 从所有光源中随机选择一个
		int index = std::min((int)(sampler->next() * lights.size()), (int)(lights.size() - 1));
